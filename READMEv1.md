...
- **`terraform.tfvars.example` is incomplete.** `public_key_path`, `server_certificate_arn`, and `root_certificate_chain_arn` are all required variables with no defaults, but the example file does not yet list them. Set all three in `terraform.tfvars` before running `plan`.
- **`ec2`, `key-pair`, `security-group`, `security-group-rule`, and `client-vpn` have no populated `versions.tf`.** They inherit constraints from the calling environment today, which works in this repository but does not pin versions if the modules are consumed standalone. Every `backup-*` module already pins `required_version` and the AWS provider correctly.
...
- **Client VPN currently authorizes access to the Recovery Access VPC only.** Core Recovery and Protected Data are not yet reachable through the VPN's authorization rules.
