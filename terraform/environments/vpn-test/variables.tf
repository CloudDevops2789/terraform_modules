variable "aws_region" {
  description = "AWS Region in which to deploy the VPN test root."
  type        = string
  default     = "us-east-1"
  nullable    = false
}
...
variable "public_key_path" {
  description = "Path to the approved SSH public key used by the test instance."
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.public_key_path)) > 0
    error_message = "public_key_path must not be empty."
  }
}

variable "server_certificate_arn" {
  description = "ARN of the approved ACM server certificate for the Client VPN endpoint."
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.server_certificate_arn)) > 0
    error_message = "server_certificate_arn must not be empty."
  }
}

variable "root_certificate_chain_arn" {
  description = "ARN of the approved ACM root certificate chain for Client VPN authentication."
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.root_certificate_chain_arn)) > 0
    error_message = "root_certificate_chain_arn must not be empty."
  }
}
